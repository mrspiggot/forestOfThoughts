...
2. This technique revolves around exploring multiple reasoning paths over thoughts. It treats the problem as a search over a tree structure, with each node representing a partial solution and the branches corresponding to operators that modify the solution. It involves thought decomposition, thought generation, state evaluation, and a search algorithm [2].
...
"""
    ...
    def determine_and_execute(self):
        llm_1 = OpenAI(temperature=0.9, max_tokens=3000)
...
def run_app():
    ...
    question = st.text_area('Enter your question here:', height=400)
